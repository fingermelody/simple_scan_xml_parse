...
void hash_map_insert(hash_map *map,void *key,void *value){
	linked_list *list = map->table[map->hash_func(key, map->capacity)];
		...
		hash_map_pair *pair = (hash_map_pair *) safe_malloc(sizeof(hash_map_pair));
		pair->key = MPQ_Generate_Key((char*)key);
		linked_list *values = (linked_list*)safe_malloc(sizeof(linked_list));
		linked_list_init(values,(linked_list_destructor)safe_free);
		linked_list_append(values,value);
		pair->value = (void*)values;
